...
def generate_formatted_html_for_tab(raw_data_rows, region):
    ...
    for item in processed_block:
        if isinstance(item, dict) and item.get('type') == 'details':
            ...
            cell_title_raw = row[title_col_idx] if len(row) > title_col_idx else ""
            cell_title = process_cell(cell_title_raw, False)
            ...
            if cell_title_lower in care_headers or (care_instructions_started and (cell_title or has_value_content)):
             ...
             st.warning(f"Warning: Unexpected item type in processed_block: {type(item)}")
    ...
    if any(s.get('title') or s.get('rows') or s.get('notes') for s in spec_sections):
        ...
        for section_idx, section in enumerate(spec_sections):
             if section.get('title'):
                 h3_style = ' style="margin-top: 30px;"' if section_idx > 0 or (section_idx == 0 and spec_sections[0].get('title') is None and any(s.get('title') for s in spec_sections)) else ''
                 specs_box_content += f'<h3{h3_style}>{section["title"]}</h3>\n'
             if section.get('rows'):
    ...
    care_tab_html = ""
    filtered_care_parts = [part for part in care_instructions_html_parts if str(part).strip()]
...
def generate_tabbed_html(tabs_data, region, auto_width_enabled, th150_width_input_value):
    ...
    * {{ font-family: nunitoregular, sans-serif; font-size: 14px; box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    try:
        ...
        st.error(f"HTML parsing/prettifying error: {e}. Returning raw HTML.")
        return html_output
    ...
    for index, row_series in df.iterrows():
        ...
        if current_sku is not None:
            if is_tab_marker:
                ...
                is_meaningful_data_row = any(c for c in row_as_list[1:]) or first_cell_value.lower() in ['start','end']
                if first_cell_value or is_meaningful_data_row :
    ...
    if current_sku is not None:
         ...
         if current_sku_tabs_data:
            try:
             ...
             status_area.info(f"Info: Last SKU '{current_sku}' had no processable specification/tab data rows.")

    if not output_rows:
         err_msg = ("Conversion finished, but NO valid SKU data resulted in HTML output.\n"
                    "Please check your input file structure against the instructions.")
...
def main():
    ...
    with col2_settings:
        th150_width_input = st.text_input("Manual Spec Header Width (if Auto unchecked)",
                                          ...
                                          help="Enter manual width (e.g., '180px', '30%', 'auto'). Overridden if 'Auto width' is checked.",
    ...
    if convert_button:
        if uploaded_file is not None:
            ...
            current_progress = progress_bar_container.progress(0)
            
            manual_width_val = th150_width_input if not auto_width_checkbox else ""
            if not auto_width_checkbox and manual_width_val:
                 if not (manual_width_val.endswith('px') or manual_width_val.endswith('%') or manual_width_val.lower() == 'auto'):
                     st.warning(f"Manual width '{manual_width_val}' might not be standard CSS. Using it as is.")
            ...
            try:
                ...
                if output_df is not None and not output_df.empty:
                    ...
                    with pd.ExcelWriter(output_buffer, engine='openpyxl') as writer:
                        output_df.to_excel(writer, index=False, sheet_name='ConvertedHTML')
                    ...
                    status_area.error("An unexpected issue occurred. No output was generated.")
                    progress_bar_container.empty()
